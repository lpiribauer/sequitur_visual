...
function getChildren(rule){
    ...
    for (var symbol = rule.first(); !symbol.isGuard(); symbol = symbol.getNext()) {
            if (symbol.getRule()) {
                ...
                node.text.name = ruleNumber;
                nodeArray.push(node);
            } else {
                node.text.name = printTerminal(symbol.value());
                nodeArray.push(node);
            }
        }
    return nodeArray;
    ...
    for (var symbol = rule.first(); !symbol.isGuard(); symbol = symbol.getNext()) {
        ...
        if (symbol.getRule()) {
            ...
            if (rules[ruleNumber]) {
                ...
                node.children = printNode(symbol.getRule());
